...
class Flickr8k:
    ...
    def __all_imgfilename_to_caplist_dict(self):
    ...
    def imgfilename_to_caplist_dict(self, img_path_list):
        d = {}
        for i in img_path_list:
            if i[len(self.images):] in self.imgname_to_caplist:
                d[i] = self.imgname_to_caplist[i[len(self.images):]]
        return d

    def get_imgpathlist(self, dist='val'):
        dist_images = set(open(self.dist_list[dist], 'r').read().strip().split('\n'))
        dist_imgpathlist = split_data(dist_images, img=self.img_fullpathlist, images=self.images)
        return dist_imgpathlist

    def add_start_end_seq(self, imgfilename_to_caplist_dict, startseq="<start> ", endseq="<end>"):
        caps = []
        for key, val in imgfilename_to_caplist_dict.items():
            for i in val:
